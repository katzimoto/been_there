...
| 5 | `verification.passed` | `verification.completed` → `verified` (Identity) | yes | yes | yes | **Critical** | States that the account is now discoverable. No provider name, no scores, no likelihood values. | `verification:{verificationId}:{subjectUserId}:passed` |
| 6 | `verification.failed` | `verification.completed` → `verification_failed` (Identity) | yes | yes | yes | **Critical** | Says which step failed in neutral terms and what to do next. **Never** says "you look like a criminal" or quotes a similarity score. | `verification:{verificationId}:{subjectUserId}:failed` |
| 7 | `verification.review_required` | `identity_status.changed` → `review_required` (Identity) | yes | yes | yes | **Critical** | States that a human is reviewing and gives an expected timeframe. | `verification:{verificationId}:{subjectUserId}:review` |
| 8 | `verification.expired` | `identity_status.changed` → `expired` (Identity) | yes | yes | yes | **Critical** | States the account is no longer discoverable and what to do to restore it. | `verification:{verificationId}:{subjectUserId}:expired` |
...
| 17 | `login.new_device` | `account.login.succeeded` with a new device fingerprint (Platform) | yes | yes | yes | **Critical** | Device label, coarse city of the login, time. No IP address in the body, no session token material. | `login:{sessionId}` |
| 18 | `account.recovery` | `account.recovery_requested` (Platform) | no | yes | no | **Critical** | Confirms a recovery attempt. Body never states whether the address exists beyond what the account-recovery flow already guarantees. | `recovery:{requestId}` |
| 19 | `account.deletion_completed` | `account.deleted` (Platform) | no | yes | no | **Critical** | States deletion completed, what was deleted, and the recovery window if one exists. | `deletion:{requestId}` |
